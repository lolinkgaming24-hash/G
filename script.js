...
class Sorcerer {
    ...
    draw() {
        ...
        if (this.rika.active) {
            ...
            ctx.beginPath(); ctx.ellipse(rx, ry - 70, 50, 90, 0, 0, Math.PI * 2); ctx.fill();
            ctx.fillStyle = '#fff'; 
            ctx.beginPath(); ctx.arc(rx + (this.dir * 15), ry - 110, 8, 0, 7); ctx.fill();
        ...
        if (this.proj.active) {
            ...
            if (this.proj.type === 'NAIL') ctx.fillRect(this.proj.x, this.proj.y - 40, 18 * this.dir, 4);
                ...
                ctx.beginPath(); 
                let sz = (this.proj.type==='PURPLE'||this.proj.type==='UZUMAKI')?45:15; 
        ...
        let aY = (this.m1T > 0 || this.fx > 0) ? cy - 45 : cy - 60;
        ctx.beginPath(); ctx.moveTo(cx, cy - 70); ctx.lineTo(cx + (this.dir * 25), aY); ctx.stroke();
        ctx.beginPath(); ctx.moveTo(cx, cy - 70); ctx.lineTo(cx - (this.dir * 15), cy - 50); ctx.stroke();
        let w = (Math.abs(this.vx) > 0.1) ? Math.sin(this.frame * 0.2) * 12 : 5;
        ctx.beginPath(); ctx.moveTo(cx, cy - 30); ctx.lineTo(cx + w, cy); ctx.stroke(); 
        ctx.beginPath(); ctx.moveTo(cx, cy - 30); ctx.lineTo(cx - w, cy); ctx.stroke();
        ctx.restore();
    }
    ...
    spec(opp) {
        ...
        switch(this.k) {
            ...
            case 'Todo': 
                let oldX = this.x; this.x = opp.x; opp.x = oldX; 
                opp.stun = 30; 
                this.spT = 450; // UPDATED: Now uses the full cooldown bar
                break;
            ...
            case 'Geto': 
                this.getoProjs = [{x:this.x, y:this.y-80, vx:this.dir*7}, {x:this.x, y:this.y-40, vx:this.dir*7}, {x:this.x, y:this.y, vx:this.dir*7}];
                this.spT = 450; break;
    ...
    update(opp) {
        ...
        if (this.rika.active) {
            this.rika.frame--;
            if (this.rika.type === 'PUNCH') {
                if (Math.abs(this.rika.x - opp.x) < 80) { opp.hp -= 45; opp.stun = 120; opp.vx = this.dir * 40; }
            } else { this.rika.x = this.x - (this.dir * 60); }
            if (this.rika.frame <= 0) this.rika.active = false;
        }

        this.getoProjs = this.getoProjs.filter(p => {
            p.x += p.vx;
            if (Math.abs(p.x-(opp.x+20))<40 && Math.abs(p.y-(opp.y-40))<60) { opp.hp -= 25; opp.stun = 15; return false; }
        ...
        if (this.fx > 0) {
            ...
            if (this.k === 'Nanami' && dist < 75) { opp.hp -= 50; opp.silence = 180; this.fx = 0; }
            if ((this.k === 'Toji' || this.k === 'Maki') && dist < 85) { opp.hp -= 5; opp.stun = 12; opp.vx = this.dir * 25; }
            
            if (isBeaming) {
                let bStart = this.x + 20, bEnd = this.dir === 1 ? bStart + 2000 : bStart - 2000;
                let mnX = Math.min(bStart, bEnd), mxX = Math.max(bStart, bEnd);
                let inX = (opp.x+20) > mnX && (opp.x+20) < mxX;
                let inY = Math.abs((this.y-35)-(opp.y-40)) < 60;
                let cl = (p1.fx>0 && p2.fx>0 && (p1.k==='Ryu'||p1.k==='Yuta') && (p2.k==='Ryu'||p2.k==='Yuta'));
                if (inX && inY && !cl) { opp.hp -= 2.2; opp.stun = 3; }
            }
        }
        ...
        if (this.proj.active) {
            ...
            if (Math.abs(this.proj.x-(opp.x+20)) < 60 && Math.abs(this.proj.y-40-(opp.y-40)) < 90) {
                if (this.proj.type === 'NAIL') { opp.hp -= 35; opp.stun = 120; }
        ...
        if (isBeaming) { this.vx = 0; this.vy = 0; } 
            ...
            let spd = this.inShadow ? this.s.s * 1.5 : this.s.s;
        ...
        if (!isBeaming) { if (this.y < gnd) this.vy += 0.85; else { this.y = gnd; this.vy = 0; } }
    ...
    atk(opp) {
        ...
        if (Math.abs(this.x - opp.x) < 90 && Math.abs(this.y - opp.y) < 100) {
            if (this.inShadow) { opp.hp -= (this.s.d + 20); opp.stun = 60; this.inShadow = false; }
    ...
    Object.keys(chars).forEach(c => {
        ...
        b.onpointerdown = (e) => {
            ...
            if (!p1C) { 
                p1C = c; 
                if (mode === '1P') { 
                    const rst = Object.keys(chars);
                    p2C = rst[Math.floor(Math.random() * rst.length)];
                    startGame(); 
...
function togglePause() {
    ...
    if (paused) s.classList.add('active-menu'); else s.classList.remove('active-menu');
}
...
window.addEventListener('touchstart', e => {
    ...
    [...e.touches].forEach(touch => {
        ...
        const pN = b.dataset.p, p = (pN === '1') ? p1 : p2, op = (pN === '1') ? p2 : p1;
        const bm = (p.fx > 0 && (p.k === 'Ryu' || p.k === 'Yuta'));
        if (b.dataset.v === 'l') held['p'+pN+'L'] = true;
        if (b.dataset.v === 'r') held['p'+pN+'R'] = true;
        if (b.dataset.v === 'u' && p.vy === 0 && !bm) p.vy = -19;
        if (b.dataset.v === 'a') p.atk(op);
        if (b.dataset.v === 's') p.spec(op);
    });
}, {passive: false});
