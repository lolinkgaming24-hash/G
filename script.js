/**
 * JUJUTSU SHOWDOWN: SORCERY UNLEASHED - ENGINE v30.0
 * -------------------------------------------------------------------------
 * MANDATE CHECKLIST:
 ...
 * 4. MENU FIX: Functions aligned for 1P/2P selection.
...
class Sorcerer {
    ...
    // RECHARGE LOGIC: Only refill if nothing is flying/casting
    ...
    processTechniqueLogic(opp) {
        if (this.key === 'Sukuna' && this.specialDuration > 0) {
            if (GAME.frame % 10 === 0) { this.spawnDismantleSlash(); }
        }
        this.updateDismantleSlashes(opp);

        if (this.activeProjectile.active) { this.updateProjectilePhysics(opp); }
    ...
    performMelee(opp) {
        ...
        this.atkRecovery = 22;
        if (Math.abs(this.x - opp.x) < 100 && !opp.isShadowStealth) {
            let damage = this.stats.attackDamage;
            if (this.isShadowStealth) { damage += 40; this.isShadowStealth = false; }
    ...
    performTechnique(opp) {
        ...
        switch (this.stats.technique) {
            case 'HOLLOW_PURPLE':
                this.activeProjectile = { active: true, x: this.x + 20, y: this.y - 50, vx: this.dir * 14, type: 'PURPLE' };
                break;
            case 'DISMANTLE':
            ...
            case 'BLOOD_PIERCING':
            case 'EMBER_INSECT':
                this.activeProjectile = { active: true, x: this.x + 20, y: this.y - 50, vx: this.dir * 22, type: 'PROJ' };
                break;
            case 'IDLE_TRANSFIG':
                let tempX = this.x; this.x = opp.x; opp.x = tempX; opp.stun = 55;
                break;
            case 'RATIO_TECH':
                if (Math.abs(this.x - opp.x) < 120) { opp.receiveDamage(148, this); opp.stun = 65; }
                break;
        }
    }
    ...
    draw(c) {
        c.save();
        const cx = this.x + 20; const cy = this.y - 50;
        if (this.isShadowStealth) {
            c.globalAlpha = 0.3; c.fillStyle = '#000';
            c.beginPath(); c.ellipse(cx, this.y, 45, 10, 0, 0, Math.PI * 2); c.fill();
        }
        c.lineWidth = ENGINE.RENDER.LINE_W; c.strokeStyle = this.stats.color;
        if (this.stun > 0) c.translate((Math.random() - 0.5) * 5, 0);
        const leg = (Math.abs(this.vx) > 0.6) ? Math.sin(GAME.frame * 0.3) * 25 : 0;
        c.beginPath(); c.arc(cx, cy - 45, ENGINE.RENDER.HEAD_RAD, 0, Math.PI * 2); c.stroke();
        c.beginPath(); c.moveTo(cx, cy - 34); c.lineTo(cx, cy + 12); c.stroke();
        let reach = (this.atkRecovery > 0) ? 42 : 22;
        c.beginPath(); c.moveTo(cx, cy - 25); c.lineTo(cx + (this.dir * reach), cy - 5); c.stroke();
        c.beginPath(); c.moveTo(cx, cy + 12); c.lineTo(cx + leg, cy + 50); c.stroke();
        c.beginPath(); c.moveTo(cx, cy + 12); c.lineTo(cx - leg, cy + 50); c.stroke();
        this.renderTechniqueAssets(c, cx, cy);
        c.restore();
    }
    ...
    renderTechniqueAssets(c, cx, cy) {
        if (this.key === 'Sukuna') {
            ...
            this.flyingSlashes.forEach(s => {
                c.beginPath(); c.moveTo(s.x, s.y - 15); c.lineTo(s.x + (this.dir * 30), s.y + 15); c.stroke();
            });
        }
        if (this.key === 'Gojo' && this.activeProjectile.active) {
            c.fillStyle = '#aa00ff'; c.shadowBlur = 35; c.shadowColor = '#aa00ff';
            c.beginPath(); c.arc(this.activeProjectile.x, this.activeProjectile.y, 35, 0, Math.PI * 2); c.fill();
        }
        if (this.specialDuration > 0 && (this.stats.technique === 'GRANITE_BLAST' || this.stats.technique === 'PURE_LOVE')) {
            let grad = c.createLinearGradient(cx, cy, cx + (1200 * this.dir), cy);
            grad.addColorStop(0, this.stats.color); grad.addColorStop(1, 'rgba(0,0,0,0)');
            c.fillStyle = grad; c.globalAlpha = 0.45; c.fillRect(cx, cy - 25, 1200 * this.dir, 50);
        }
    }

    processAi(opp) {
        let dist = Math.abs(this.x - opp.x); this.dir = (opp.x < this.x) ? -1 : 1;
        if (dist > 130) { this.vx = (opp.x < this.x ? -this.stats.speed : this.stats.speed) * 0.72; } 
        else { if (Math.random() < 0.05) this.performMelee(opp); }
        if (this.sp >= ENGINE.STATS.MAX_SP && Math.random() < 0.02) { this.performTechnique(opp); }
    }
}
...
function gameLoop() {
    ...
    GAME.vfxPool = GAME.vfxPool.filter(p => { p.update(); p.draw(ctx); return p.alpha > 0; });
    GAME.p1.update(GAME.p2); GAME.p2.update(GAME.p1);
    GAME.p1.draw(ctx); GAME.p2.draw(ctx);
...
window.onload = () => { document.getElementById('m-start').classList.add('active-menu'); };
